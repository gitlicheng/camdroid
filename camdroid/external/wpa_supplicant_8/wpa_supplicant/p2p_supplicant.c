...
//	if (wpa_s->go_params)
//	{	
//		wpa_printf(MSG_INFO, "im here %s, line: %d", __FUNCTION__,__LINE__);
//		bssid = wpa_s->go_params->peer_interface_addr;
//		wpa_printf(MSG_INFO, "bssid is " MACSTR " ", MAC2STR(bssid));
//	}	
//	else
	{
//		wpa_printf(MSG_INFO, "im here %s, line: %d", __FUNCTION__,__LINE__);
...
void wpas_p2p_completed(struct wpa_supplicant *wpa_s)
{
	...
	//added by JM.@2013-05-13
	if(is_zero_ether_addr(go_dev_addr) &&  wpa_s->bssid != NULL){
		os_memcpy(go_dev_addr, wpa_s->bssid, ETH_ALEN);		
	}
