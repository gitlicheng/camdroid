...
#define ANDROID_LOG_NAME	"p2p0-dnsmasq"
#endif /* ANDROID_LOG_NAME */
#define DEGUB_LEVLE_1 1
  ...
  */
  ...
  switch (mess_type)
      ...
      else
	{
	  ...
	  {
        struct in_addr a;
        void *addr1 = (void *)&mess->yiaddr;
        if(addr1) {
            memcpy(&a, addr1, sizeof(a));
            #define WFD_DNSMASQ_PEER_IP  "wlan.wfddnsmasq.peer"
            android_printf(DEGUB_LEVLE_1, "client ip=%s", addr1 ? inet_ntoa(a) : "none");
            property_set(WFD_DNSMASQ_PEER_IP, addr1 ? inet_ntoa(a) : "none");
        }
	  }
