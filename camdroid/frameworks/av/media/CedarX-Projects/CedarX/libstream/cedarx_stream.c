...
struct cdx_stream_info *create_stream_handle(CedarXDataSourceDesc *datasource_desc)
{
	...
	if(datasource_desc->thirdpart_stream_type != CEDARX_THIRDPART_STREAM_NONE && datasource_desc->stream_type != CEDARX_STREAM_NETWORK) 
    {
		ret = thirdpart_create_stream_handle(stm_info);
	}
    else if((datasource_desc->source_type == CEDARX_SOURCE_FILEPATH || datasource_desc->source_type == CEDARX_SOURCE_FD)
