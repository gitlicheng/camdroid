                               ...
                               int av_sync)
{
    ...
    switch (instr)
    {
        ...
        case DSP_INSTR_MERGE:
            ...
            ALOGV("WebRtcNetEQ_Merge(%d): return_value(%d), len(%d)",
                    __LINE__, return_value, len);
            if (return_value < 0)
            {
                /* error */
                return return_value;
        ...
        case DSP_INSTR_EXPAND:
                ...
                < (inst->timestampsPerCall))
            {
                ...
                ALOGV("WebRtcNetEQ_Expand(%d): return_value(%d), len(%d)",
                        __LINE__, return_value, len);
                if (return_value < 0)
                {
                    /* error */
        ...
        case DSP_INSTR_ACCELERATE:
            if (len < 3 * 80 * fs_mult)
            {
                ...
                return_value = WebRtcNetEQ_Accelerate(inst,
                    ...
                    pw16_NetEqAlgorithm_buffer, &len, BGNonly);
                ALOGV("WebRtcNetEQ_Accelerate(%d): return_value(%d), len(%d)",
                        __LINE__, return_value, len);
                if (return_value < 0)
                {
                    /* error */
        ...
        case DSP_INSTR_FADE_TO_BGN:
        {
            ...
            ALOGV("WebRtcNetEQ_Expand(%d): tempReturnValue(%d), len(%d)",
                    __LINE__, tempReturnValue, len);
