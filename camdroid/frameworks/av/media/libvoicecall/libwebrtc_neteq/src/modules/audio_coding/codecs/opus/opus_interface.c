...
//#define OPUS_DEC_DEBUG
#ifdef OPUS_DEC_DEBUG
...
int16_t WebRtcOpus_EncoderCreate(OpusEncInst** inst, int32_t sampleRate, int32_t channels) {
  OpusEncInst* state;
  if (inst != NULL) {
    state = (OpusEncInst*) calloc(1, sizeof(OpusEncInst));
    if (state) {
...
int16_t WebRtcOpus_EnableVbr(OpusEncInst* inst) {
  if (inst) {
...
int16_t WebRtcOpus_DisableVbr(OpusEncInst* inst) {
  if (inst) {
...
int16_t WebRtcOpus_DecoderCreate(OpusDecInst** inst, int sampleRate, int channels) {
  int error_l;
  int error_r;
  OpusDecInst* state;
...
#ifdef OPUS_DEC_DEBUG
    pcm_file = fopen("/mnt/extsd/opus_dec.pcm", "w+");
#endif
    /* Create new memory for left and right channel, always at 48000 Hz. */
    state->decoder_left = opus_decoder_create(sampleRate, channels, &error_l);
    state->decoder_right = opus_decoder_create(sampleRate, channels, &error_r);
    if (error_l == OPUS_OK && error_r == OPUS_OK && state->decoder_left != NULL
        && state->decoder_right != NULL) {
      /* Creation of memory all ok. */
      state->sample_rate = sampleRate;
      state->channels = channels;
      state->prev_decoded_samples = 20 * sampleRate / 1000;  //kWebRtcOpusDefaultFrameSize;
      *inst = state;
      return 0;
    }
...
int16_t WebRtcOpus_DecodeNoneResample(OpusDecInst* inst, const int16_t* encoded,
                          int16_t encoded_bytes, int16_t* decoded,
                          int16_t* audio_type) {
  /* |buffer16| is big enough for 120 ms (the largestOpus packet size) of
   * stereo audio at 48 kHz. */
  ...
  decoded_samples = DecodeNative(inst->decoder_left, encoded, encoded_bytes,
                                 ...
                                 audio_type);
  ALOGV("WebRtcOpus_DecodeNoneResample: decoded_samples(%d)",
        decoded_samples);
...
#ifdef OPUS_DEC_DEBUG
    if (pcm_file) {
        fwrite((char *)decoded, 2,decoded_samples, pcm_file);
    }
#endif    
...
int16_t WebRtcOpus_DecodePlcNoneResample(OpusDecInst* inst, int16_t* decoded,
                             int16_t number_of_lost_frames) {
  int16_t buffer[kWebRtcOpusMaxFrameSize];
  int16_t audio_type = 0;
  int decoded_samples;
  ...
  decoded_samples = DecodeNative(inst->decoder_left, NULL, 0, plc_samples,
                                 decoded, &audio_type);
  ALOGV("WebRtcOpus_DecodePlcNoneResample: number_of_lost_frames(%d), decoded_samples(%d)",
        number_of_lost_frames, decoded_samples);
  if (decoded_samples < 0) {
    return -1;
  }
#if 0  
...
int WebRtcOpus_DurationEstNoneResample(OpusDecInst* inst,
                           const uint8_t* payload,
                           int payload_length_bytes) {
  int frames, samples;
  frames = opus_packet_get_nb_frames(payload, payload_length_bytes);
  ...
  samples = frames * opus_packet_get_samples_per_frame(payload, inst->sample_rate);
  if (samples < 120 || samples > 5760) {
    /* Invalid payload duration. */
    return 0;
  }
