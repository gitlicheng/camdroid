...
private:
    ...
    public:
        ...
        struct record_track_buffer_manager_t {
                        void *      buffers;
                        uint32_t    bufferSize;
                        uint32_t    availSize;
        ...
        public:
            ...
            static  void        appendDumpHeader(String8& result);
                    ...
                    /*added by ywl for multi recorder --begin*/
                    status_t initInputParameters();
                    status_t reConfigNewParameters();
                    bool checkForNewParameters();

                    class TrackThread : public Thread {
                    public:
                        TrackThread(const sp<RecordTrack>& recordTrack);
                    ...
                    bool processAudioBuffer(const sp<TrackThread>& thread);

                    class ResamplerBufferProvider : public AudioBufferProvider {
                    public :
                        ResamplerBufferProvider(const sp<RecordTrack>& recordTrack){
                    ...
                    status_t RsmpGetNextBuffer(AudioBufferProvider::Buffer* buffer, int64_t pts);
                    void RsmpReleaseBuffer(AudioBufferProvider::Buffer* buffer);
                    /*added by ywl for multi recorder --end*/
        ...
        private:
            ...
            /*added by ywl for multi recorder --begin*/
            status_t    checkAvailBuffer(size_t size);
            status_t    checkAvailBuffer_l(size_t size);
			status_t    checkAudioBuffer(size_t size);
            status_t    fillAudioBuffer(void *buffer, size_t size);
            size_t      getAudioBuffer(void *buffer, size_t size);

            record_track_buffer_manager_t       mPcmBufManager;

            bool                    mReConfig;
            sp<TrackThread>         mTrackThread;
            uint32_t                mInputSampleRate;
            size_t                  mInputFrameCount;       // output HAL, direct output, record
            size_t                  mInputNormalFrameCount; // normal mixer and effects
            audio_channel_mask_t    mInputChannelMask;
            uint16_t                mInputChannelCount;
            size_t                  mInputFrameSize;
            audio_format_t          mInputFormat;
            int16_t *               mInputRsmpInBuffer;
            size_t                  mInputRsmpInIndex;
            size_t                  mInputBufferSize;
            ssize_t                 mInputBytesRead;
            Mutex                   mLock;
            Condition               mCond;
            AudioResampler          *mResampler;
            ResamplerBufferProvider *mRsmBufProvider;
            int32_t *               mRsmpOutBuffer;
            //uint32_t                mReqChannelCount;
            //uint32_t                mReqSampleRate;
            //audio_format_t          mReqFormat;
            /*added by ywl for multi recorder --end*/
        };
