        ...
        status_t *status)
{
    ...
    if (mType == DIRECT) {
        ...
        if (sampleRate > 48000*2) {
            ALOGE("Sample rate out of range: %d mSampleRate %d", sampleRate, 48000*2);
            lStatus = BAD_VALUE;
            goto Exit;
        }
    }
        ...
        mOverflow(false)
{
    ...
    /*added by ywl for multi recorder --begin*/
    mResampler         = NULL;
    mRsmBufProvider    = NULL;
    mRsmpOutBuffer     = NULL;
    mInputRsmpInBuffer = NULL;
    memset(&mPcmBufManager, 0, sizeof(record_track_buffer_manager_t));
    mReConfig = false;

    status_t ret = initInputParameters();
    if (ret != NO_ERROR) {
        ALOGE("RecordTrack: initInputParameters failed.");
    }

    mTrackThread = new TrackThread(this);
    mTrackThread->run("RecordTrack", ANDROID_PRIORITY_URGENT_AUDIO);
    /*added by ywl for multi recorder --end*/
}
...
AudioFlinger::RecordThread::RecordTrack::~RecordTrack()
{
    ALOGV("%s", __func__);
    /*added by ywl for multi recorder --begin*/
    delete[] mInputRsmpInBuffer;
    delete mResampler;
    delete[] mRsmpOutBuffer;

    if (mPcmBufManager.buffers) {
        free(mPcmBufManager.buffers);
        mPcmBufManager.buffers = NULL;
    }
    /*added by ywl for multi recorder --end*/
}
...
bool AudioFlinger::RecordThread::RecordTrack::processAudioBuffer(const sp<TrackThread>& thread)
{
    AudioBufferProvider::Buffer buffer;
    static nsecs_t lastWarning = 0;

    //ALOGV("processAudioBuffer");

    if (mState != TrackBase::ACTIVE) {
    ...
    if (checkAudioBuffer(mInputBufferSize) != OK) {
        //usleep(kRecordThreadSleepUs);
        AutoMutex _l(mLock);
        mCond.wait(mLock);
        return true;
    ...
    buffer.frameCount = mInputFrameCount;
    if (CC_LIKELY(getNextBuffer(&buffer) == NO_ERROR)) {
        size_t framesOut = buffer.frameCount;
        if (mResampler == NULL) {
            // no resampling
            while (framesOut) {
                size_t framesIn = mInputFrameCount - mInputRsmpInIndex;;
                if (framesIn) {
                    int8_t *src = (int8_t *)mInputRsmpInBuffer + mInputRsmpInIndex * mInputFrameSize;
                	int8_t *dst = buffer.i8 + (buffer.frameCount - framesOut) * mCblk->frameSize;
                    if (framesIn > framesOut)
                        framesIn = framesOut;
                    mInputRsmpInIndex += framesIn;
                    framesOut -= framesIn;
                    if (mInputChannelCount == mChannelCount) {
                        memcpy(dst, src, framesIn * mInputFrameSize);
                    } else {
                ...
                if (framesOut && mInputFrameCount == mInputRsmpInIndex) {
                    void *readInto;
                    if (framesOut == mInputFrameCount && mInputChannelCount == mChannelCount) {
                        readInto = buffer.raw;
                        framesOut = 0;
                    } else {
                        readInto = mInputRsmpInBuffer;
                        mInputRsmpInIndex = 0;
                    }

                    mInputBytesRead = getAudioBuffer(readInto, mInputBufferSize);
                    if (mInputBytesRead <= 0) {
                        if ((mInputBytesRead < 0) && (mState == TrackBase::ACTIVE))
                        {
                        ...
                        mInputRsmpInIndex = mInputFrameCount;
                        framesOut = 0;
                        buffer.frameCount = 0;
                    }
                }
    ...
    // client isn't retrieving buffers fast enough
        ...
        //ALOGW("processAudioBuffer: getNextBuffer error");
        if (!setOverflow()) {           //continue overflow
            nsecs_t now = systemTime();
            if ((now - lastWarning) > kWarningThrottleNs) {
                ALOGW("RecordThread(%d): buffer overflow", gettid());
                lastWarning = now;
            }
        }
...
status_t AudioFlinger::RecordThread::RecordTrack::initInputParameters()
{
    ...
    if (thread != 0) {
        ...
        ALOGI("RecordTrack: mSampleRate(%d), mChannelCount(%d)", 
            mSampleRate, mChannelCount);

        mInputFrameCount = recordThread->mFrameCount;
        mInputSampleRate = recordThread->mSampleRate;
        mInputChannelMask = recordThread->mChannelMask;
        mInputChannelCount = recordThread->mChannelCount;
        mInputFormat = recordThread->mFormat;
        mInputFrameSize = recordThread->mFrameSize;
        mInputBufferSize = recordThread->mInputBytes;

        mInputRsmpInBuffer = new int16_t[mInputFrameCount * mInputChannelCount];
        if (!mInputRsmpInBuffer) {
            return BAD_VALUE;
        }
        
        mPcmBufManager.bufferSize = mInputBufferSize * 10;
        mPcmBufManager.buffers = malloc(mPcmBufManager.bufferSize);
        if (!mPcmBufManager.buffers) {
            delete[] mInputRsmpInBuffer;
            mInputRsmpInBuffer = NULL;
            return BAD_VALUE;
        }
        mPcmBufManager.availSize = mPcmBufManager.bufferSize;
        mPcmBufManager.user = (uint32_t)mPcmBufManager.buffers;
        mPcmBufManager.server = (uint32_t)mPcmBufManager.buffers;

        if (mInputSampleRate != mSampleRate && mInputChannelCount <= FCC_2 && mChannelCount <= FCC_2)
        {
            int channelCount;
            // optimization: if mono to mono, use the resampler in stereo to stereo mode to avoid
            // stereo to mono post process as the resampler always outputs stereo.
            if (mInputChannelCount == 1 && mChannelCount == 2) {
                channelCount = 1;
            } else {
                channelCount = 2;
            }
            mResampler = AudioResampler::create(16, channelCount, mSampleRate);
            mResampler->setSampleRate(mInputSampleRate);
            mResampler->setVolume(AudioMixer::UNITY_GAIN, AudioMixer::UNITY_GAIN);
            mRsmpOutBuffer = new int32_t[mInputFrameCount * FCC_2];
            ...
            mRsmBufProvider = new ResamplerBufferProvider(this);
        }
        

        mInputRsmpInIndex = mInputFrameCount;
        
        ALOGV("initInputParameters: mInputFrameCount(%d), mInputRsmpInIndex(%d)", mInputFrameCount, mInputRsmpInIndex);
...
status_t AudioFlinger::RecordThread::RecordTrack::reConfigNewParameters()
{
    AutoMutex lock(mLock);
    mReConfig = true;
...
bool AudioFlinger::RecordThread::RecordTrack::checkForNewParameters()
{
    AutoMutex lock(mLock);
...
status_t AudioFlinger::RecordThread::RecordTrack::checkAvailBuffer(size_t size)
{
    AutoMutex lock(mLock);

    return checkAvailBuffer_l(size);
}

status_t AudioFlinger::RecordThread::RecordTrack::checkAvailBuffer_l(size_t size)
{
    //ALOGV("checkAvailBuffer_l: availSize(%d), size(%d)", mPcmBufManager.availSize, size);
    if (mPcmBufManager.availSize >= size)
        return OK;
    else 
        return NOT_ENOUGH_DATA;
}

status_t AudioFlinger::RecordThread::RecordTrack::checkAudioBuffer(size_t size)
{
    size_t bufferSize = mPcmBufManager.bufferSize - mPcmBufManager.availSize;
...
status_t AudioFlinger::RecordThread::RecordTrack::fillAudioBuffer(void *buffer, size_t size)
{
    AutoMutex lock(mLock);

    //ALOGV("fillAudioBuffer");

    uint32_t buf_end = (uint32_t)mPcmBufManager.buffers + mPcmBufManager.bufferSize;

    status_t ret = checkAvailBuffer_l(size);
    if (ret != OK) {
        return ret;
    }
/*
    if (mPcmBufManager.server + size > buf_end) {
        uint32_t write_size = buf_end - mPcmBufManager.server;
        memcpy((void *)mPcmBufManager.server, buffer, write_size);
        memcpy(mPcmBufManager.buffers, (char *)buffer + write_size, size - write_size);

        mPcmBufManager.server = (uint32_t)mPcmBufManager.buffers + size - write_size;       
    } else {    
        memcpy((void *)mPcmBufManager.server, buffer, size);
        mPcmBufManager.server += size; 
    }
*/
    memcpy((void *)mPcmBufManager.server, buffer, size);
    mPcmBufManager.server += size; 
    if (mPcmBufManager.server == buf_end) {
        mPcmBufManager.server = (uint32_t)mPcmBufManager.buffers;
    }
    mPcmBufManager.availSize -= size;

    mCond.signal();
    return OK;
}

size_t AudioFlinger::RecordThread::RecordTrack::getAudioBuffer(void *buffer, size_t size)
{
    ...
    //ALOGV("getAudioBuffer: bufferSize(%d), size(%d)", bufferSize, size);
/*
    if (mPcmBufManager.user + size > buf_end) {
        uint32_t read_size = buf_end - mPcmBufManager.user;
        memcpy(buffer, (void *)mPcmBufManager.user, read_size);
        memcpy((char *)buffer + read_size, mPcmBufManager.buffers, size - read_size);
        mPcmBufManager.user = (uint32_t)mPcmBufManager.buffers + size - read_size;
    } else {
        memcpy(buffer, (void *)mPcmBufManager.user, size);
        mPcmBufManager.user += size; 
    }
*/
    memcpy(buffer, (void *)mPcmBufManager.user, size);
    mPcmBufManager.user += size;
    if (mPcmBufManager.user == buf_end) {
        mPcmBufManager.user = (uint32_t)mPcmBufManager.buffers;
    }
    mPcmBufManager.availSize += size;
...
status_t AudioFlinger::RecordThread::RecordTrack::RsmpGetNextBuffer(AudioBufferProvider::Buffer* buffer,
        int64_t pts)
{
    size_t framesReq = buffer->frameCount;
    size_t framesReady = mInputFrameCount - mInputRsmpInIndex;
    int channelCount;

    ALOGV("RsmpGetNextBuffer");
    
    if (framesReady == 0) {
        mInputBytesRead = getAudioBuffer(mInputRsmpInBuffer, mInputBufferSize);
        if (mInputBytesRead <= 0) {
            if ((mInputBytesRead < 0) && (mState == TrackBase::ACTIVE))
            {
    ...
    buffer->raw = mInputRsmpInBuffer + mInputRsmpInIndex * channelCount;
    buffer->frameCount = framesReq;
    return NO_ERROR;
}

void AudioFlinger::RecordThread::RecordTrack::RsmpReleaseBuffer(AudioBufferProvider::Buffer* buffer)
{
    ALOGV("RsmpReleaseBuffer");

    mInputRsmpInIndex += buffer->frameCount;
    buffer->frameCount = 0;
}
/*added by ywl for multi recorder --end*/
                                                        ...
                                                        int triggerSession)
{
    ...
    if (thread != 0) {
        RecordThread *recordThread = (RecordThread *)thread.get();
        /*modified by ywl for multi recorder --begin*/
        //return recordThread->start(this, event, triggerSession);
        status_t ret = recordThread->start(this, event, triggerSession);
        if (ret != NO_ERROR) {
            return ret;
        }
        ...
        return NO_ERROR;
        /*modified by ywl for multi recorder --end*/
    } else {
        return BAD_VALUE;
    }
}
...
/*added by ywl for multi recorder --begin*/
AudioFlinger::RecordThread::RecordTrack::TrackThread::TrackThread(const sp<RecordTrack>& recordTrack)
    : Thread(false /*canCallJava*/), mRecordTrack(recordTrack), mPaused(true)
{

}

AudioFlinger::RecordThread::RecordTrack::TrackThread::~TrackThread()
{

}

bool AudioFlinger::RecordThread::RecordTrack::TrackThread::threadLoop()
{

    //ALOGE("TrackThread::threadLoop()");
    {
    ...
    //ALOGE("TrackThread::threadLoop(): processAudioBuffer");

    if (!mRecordTrack->processAudioBuffer(this)) {
        pause();
    }
...
void AudioFlinger::RecordThread::RecordTrack::TrackThread::requestExit()
{
...
void AudioFlinger::RecordThread::RecordTrack::TrackThread::pause()
{
    AutoMutex _l(mMyLock);
    mPaused = true;
}

void AudioFlinger::RecordThread::RecordTrack::TrackThread::resume()
{
    AutoMutex _l(mMyLock);
    if (mPaused) {
        mPaused = false;
...
status_t AudioFlinger::RecordThread::RecordTrack::ResamplerBufferProvider::getNextBuffer(
    AudioBufferProvider::Buffer* buffer, int64_t pts)
{
    return mRecordTrack->RsmpGetNextBuffer(buffer, pts);
}

void AudioFlinger::RecordThread::RecordTrack::ResamplerBufferProvider::releaseBuffer(
    AudioBufferProvider::Buffer* buffer)
{
    mRecordTrack->RsmpReleaseBuffer(buffer);
}
/*added by ywl for multi recorder --end*/
    ...
    while (!exitPending()) {
        ...
        { // scope for mLock
            ...
            if (!mActiveTracks.isEmpty()) {
                size_t pauseTrack = 0;
                size_t resumeTrack = 0;
                SortedVector < sp<RecordTrack> >  tracksToRemove;
                for (size_t i = 0; i < mActiveTracks.size(); i++) {
                    sp<RecordTrack> activeTrack = mActiveTracks[i];
                    if (activeTrack->mState == TrackBase::PAUSING) {
                        tracksToRemove.add(activeTrack);
                        pauseTrack++;
                    } else if (activeTrack->mState == TrackBase::RESUMING) {
                        //if (mReqChannelCount != activeTrack->channelCount()) {     //fix me
                        //    tracksToRemove.add(activeTrack);
                        //} else {
                        ...
                        mStandby = false;
                        resumeTrack++;
                    } else if (activeTrack->mState == TrackBase::TERMINATED) {
                        tracksToRemove.add(activeTrack);
                        removeTrack_l(activeTrack);
                    }
                }

                if (pauseTrack == mActiveTracks.size()) {
                    ALOGV("standby()");
                    //readOnce = false;
                    standby();
                }
                ...
                if (!tracksToRemove.isEmpty()) {
                    ALOGV("tracksToRemove.size() is %d", tracksToRemove.size());
                    for (size_t i = 0; i < tracksToRemove.size(); i++) {
                        sp<RecordTrack> track = tracksToRemove[i];
                        mActiveTracks.remove(track);
                    }
                    tracksToRemove.clear();
                }

                if (pauseTrack || resumeTrack) {
                    ALOGV("send start/stop braodcast");
                    mStartStopCond.broadcast();
                }
            }
...

        {
            ...
            if (!mActiveTracks.isEmpty()) {
                {
                    size_t i;
                    for (i = 0; i < mActiveTracks.size(); i++) {
                        sp<RecordTrack> activeTrack = mActiveTracks[i];
                ...
                mBytesRead = mInput->stream->read(mInput->stream, mRsmpInBuffer, mInputBytes);
                mLock.lock();
                if (mBytesRead > 0 && !mActiveTracks.isEmpty()) {
                    //readOnce = true;
                    for (size_t i = 0; i < mActiveTracks.size(); i++) {
                        sp<RecordTrack> activeTrack = mActiveTracks[i];
                        if (activeTrack->mState != TrackBase::ACTIVE &&
                        ...
                        status_t ret = activeTrack->fillAudioBuffer(mRsmpInBuffer, mInputBytes);
                        if (ret != OK) {
                            ALOGD("mActiveTracks[%d] has not enough buffer", i);
                        }
                    }
                }
            }
    ...
    */
    {
        ...
        if (!mActiveTracks.isEmpty()) {
            for (size_t i = 0; i < mActiveTracks.size(); i++) {
                if (recordTrack == mActiveTracks[i].get()) {
        ...
        recordTrack->mState = TrackBase::IDLE;
        sp<RecordTrack> activeTrack = recordTrack;
        mLock.unlock();
        status_t status = AudioSystem::startInput(mId);
        mLock.lock();
        if (status != NO_ERROR) {
        ...
        activeTrack->mInputRsmpInIndex = activeTrack->mInputFrameCount;
        activeTrack->mInputBytesRead = 0;
        if (activeTrack->mResampler != NULL) {
            activeTrack->mResampler->reset();
        }
        activeTrack->mState = TrackBase::RESUMING;

        mActiveTracks.add(activeTrack);
        // signal thread to start
        ALOGV("Signal record thread");
        mWaitWorkCV.broadcast();
        // do not wait for mStartStopCond if exiting
...
bool AudioFlinger::RecordThread::stop_l(RecordThread::RecordTrack* recordTrack) {
    ...
    if (mActiveTracks.isEmpty()) {
        ...
        for (i = 0; i < mActiveTracks.size(); i++) {
            sp<RecordTrack> activeTrack = mActiveTracks[i];
            if (recordTrack == activeTrack.get()) {
                break;
            }
        }
    ...
    //}
    //return false;
    if (exitPending() || mActiveTracks.isEmpty()) {
        ALOGV("Record stopped OK");
        return true;
    }
    for (size_t i = 0; i < mActiveTracks.size(); i++) {
...
void AudioFlinger::RecordThread::RecordTrack::destroy()
{
    ...
    sp<RecordTrack> keep(this);
    {
        ...
        /*added by ywl for multi recorder --begin*/
        if (mTrackThread != NULL ) {
            ALOGV("TrackThread Exit.");
            mTrackThread->requestExit();
            {
                AutoMutex _l(mLock);
                mCond.signal();
            }
            mTrackThread->requestExitAndWait();
            mTrackThread.clear();
        }

        if (mRsmBufProvider != NULL) {
            delete mRsmBufProvider;
            mRsmBufProvider = NULL;
        }
        /*added by ywl for multi recorder --end*/
    }
...
/*
    ...
    return NO_ERROR;
*/
    ALOGE("RecordThread::getNextBuffer");
    return NOT_ENOUGH_DATA;
}
...
/*removed by ywl for multi recorder --begin*/
#if 0
...
#endif
/*removed by ywl for multi recorder --end*/
                                          ...
                                          audio_channel_mask_t *pChannelMask)
{
    ...
    /*added by ywl for multi recorder --begin*/
#ifdef CONFIG_DEFALUT_SAMPLERATE
    ALOGI("openInput(): CONFIG_DEFALUT_SAMPLERATE(%d)", CONFIG_DEFALUT_SAMPLERATE);
    config.sample_rate = CONFIG_DEFALUT_SAMPLERATE;
#else
/*
    if (config.sample_rate % SAMPLING_RATE_8K) {
        config.sample_rate = SAMPLING_RATE_16K * reqSamplingRate / SAMPLING_RATE_11K;
        if (config.sample_rate > SAMPLING_RATE_48K) {
            config.sample_rate = SAMPLING_RATE_48K;
        }
    } else {
        config.sample_rate = reqSamplingRate;
    }
*/
    config.sample_rate = reqSamplingRate;
    ALOGI("openInput(): reqSamplingRate(%d), config.sample_rate(%d)",
        reqSamplingRate, config.sample_rate);
#endif
    //use AUDIO_CHANNEL_IN_MONO shound change the config.channels to config.in_init_channels
    config.channel_mask = AUDIO_CHANNEL_IN_MONO;//AUDIO_CHANNEL_IN_STEREO;
    config.format = AUDIO_FORMAT_PCM_16_BIT;
    /*added by ywl for multi recorder --end*/
