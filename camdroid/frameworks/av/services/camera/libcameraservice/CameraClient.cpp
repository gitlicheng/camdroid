...
void CameraClient::disconnect() {
    ...
    if (mPreviewLayerId != -1) {
        //disconnectWindow(mPreviewWindow);
        mPreviewLayerId = -1;
        mHardware->setPreviewWindow(mPreviewLayerId);
    }
    mHardware.clear();
...
status_t CameraClient::setPreviewWindow(const int hlay) 
{
    Mutex::Autolock lock(mLock);
...
//        if (hlay >= 0) {
			  LOG1("(f:%s, l:%d) hlay %d", __FUNCTION__, __LINE__, hlay);
//            native_window_set_scaling_mode(window.get(),
//                    NATIVE_WINDOW_SCALING_MODE_SCALE_TO_WINDOW);
//            native_window_set_buffers_transform(window.get(), mOrientation);
            result = mHardware->setPreviewWindow(hlay);
//        }
    }
...
//        mPreviewWindow = window;
        mPreviewLayerId = hlay;
    } else {
        // Something went wrong after we connected to the new window, so
        // disconnect here.
        //disconnectWindow(window);
        ALOGE("(f:%s, l:%d) why mHardware->setPreviewWindow failed[%d], need disconnectWindow()", __FUNCTION__, __LINE__, result);
    }
...
status_t CameraClient::setPreviewDisplay(const int hlay) 
{
    //LOG1("setPreviewDisplay(%p) (pid %d)", surface.get(), getCallingPid());
    LOG1("setPreviewDisplay(%d) (pid %d)", hlay, getCallingPid());

//    sp<IBinder> binder(surface != 0 ? surface->asBinder() : 0);
//    sp<ANativeWindow> window(surface);
    return setPreviewWindow(hlay);
}

// set the SurfaceTexture that the preview will use
//status_t CameraClient::setPreviewTexture(
//        const sp<ISurfaceTexture>& surfaceTexture) 
status_t CameraClient::setPreviewTexture(const int hlay) 
{
//    LOG1("setPreviewTexture(%p) (pid %d)", surfaceTexture.get(),
//            getCallingPid());
    LOG1("setPreviewTexture(%d) (pid %d)", hlay,
        getCallingPid());
...
status_t CameraClient::startCameraMode(camera_mode mode) {
    ...
    switch(mode) {
        case CAMERA_PREVIEW_MODE:
            ...
            {
                LOG1("mSurface is not set yet.mPreviewLayerId[%d]", mPreviewLayerId);
...
//    }
    if(mPreviewLayerId < 0)
    {
        ALOGD_IF(1, "(f:%s, l:%d) PreviewLayerId[%d] is invalid", __FUNCTION__, __LINE__, mPreviewLayerId);
    }
    ALOGD("(f:%s, l:%d) PreviewLayerId[%d]", __FUNCTION__, __LINE__, mPreviewLayerId);
    mHardware->setPreviewWindow(mPreviewLayerId);
...
status_t CameraClient::getV4l2QueryCtrl(struct v4l2_queryctrl *ctrl, int id) {
    LOG1("getV4l2QueryCtrl (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->getV4l2QueryCtrl(ctrl, id);
}


int CameraClient::getInputSource(int *source) {
    LOG1("getInputSource (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->getInputSource(source);
}


int CameraClient::getTVinSystem(int *system) {
    LOG1("getTVinSystem (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->getTVinSystem(system);
}


status_t CameraClient::setWaterMark(int enable, const char *str) {
    LOG1("setWaterMark (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->setWaterMark(enable, str);
}

status_t CameraClient::setUvcGadgetMode(int value)
{
    LOG1("setUvcGadgetMode (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->setUvcGadgetMode(value);
}

status_t CameraClient::adasSetGsensorData(int val0, float val1, float val2)
{
    LOG1("adasSetGsensorData (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;

    return mHardware->adasSetGsensorData(val0, val1, val2);
}
...
//            }
            if(mPreviewLayerId >= 0)
            {
                ALOGD("(f:%s, l:%d) why call here, hwLayer not support set orientation!", __FUNCTION__, __LINE__);
            }
        }
        return OK;
...
void CameraClient::dataCallback(int32_t msgType,
    ...
    switch (msgType & ~CAMERA_MSG_PREVIEW_METADATA) {
        ...
        case CAMERA_MSG_COMPRESSED_IMAGE:
            client->handleCompressedPicture(dataPtr, metadata);
            break;
        default:
...
status_t CameraClient::dataCallbackTimestamp(nsecs_t timestamp,
        int32_t msgType, const sp<IMemory>& dataPtr, int bufIdx, void* user) {
    LOG2("dataCallbackTimestamp(%d)", msgType);

    Mutex* lock = getClientLockFromCookie(user);
    if (lock == NULL) return NO_INIT;
    Mutex::Autolock alock(*lock);

    CameraClient* client =
            static_cast<CameraClient*>(getClientFromCookie(user));
    if (client == NULL) return NO_INIT;

    if (!client->lockIfMessageWanted(msgType)) return PERMISSION_DENIED;
    ...
    client->handleGenericDataTimestamp(timestamp, msgType, dataPtr, bufIdx);
    return NO_ERROR;
}
...
void CameraClient::handleCompressedPicture(const sp<IMemory>& mem, void *metadata) {
    if ((int)metadata == 1) {
        //ALOGD("disable msg CAMERA_MSG_COMPRESSED_IMAGE");
...
void CameraClient::handleGenericDataTimestamp(nsecs_t timestamp,
    int32_t msgType, const sp<IMemory>& dataPtr, int bufIdx) {
    sp<ICameraClient> c = mCameraClient;
    mLock.unlock();
    if (c != 0) {
        c->dataCallbackTimestamp(timestamp, msgType, dataPtr, bufIdx);
    }
}
