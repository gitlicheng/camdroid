...
typedef status_t (*data_callback_timestamp)(nsecs_t timestamp,
                            int32_t msgType,
                            const sp<IMemory> &dataPtr,
                            int bufIdx,
                            void *user);
...
public:
    ...
    status_t setPreviewWindow(const int hlay)
    {
        ...
        if (mDevice->ops->set_preview_window) {
            ...
            return mDevice->ops->set_preview_window(mDevice, (struct preview_stream_ops*)hlay);
        }
	...
	status_t getV4l2QueryCtrl(struct v4l2_queryctrl *ctrl, int id) {
        ALOGV("%s(%s)", __FUNCTION__, mName.string());
        if (mDevice->ops->get_v4l2_query_ctrl)
            return mDevice->ops->get_v4l2_query_ctrl(mDevice, ctrl, id);
        return INVALID_OPERATION;
	}

	int getInputSource(int *source) {
        ALOGV("%s(%s)", __FUNCTION__, mName.string());
        if (mDevice->ops->get_input_source)
            return mDevice->ops->get_input_source(mDevice, source);
        return INVALID_OPERATION;
	}

	int getTVinSystem(int *system) {
        ALOGV("%s(%s)", __FUNCTION__, mName.string());
        if (mDevice->ops->get_tvin_system)
            return mDevice->ops->get_tvin_system(mDevice, system);
        return INVALID_OPERATION;
	}

	status_t setWaterMark(int enable, const char *str)
	{
		ALOGV("%s(%d, %s)", __FUNCTION__, enable, str);
		if(mDevice->ops->set_waterMark) {
			return mDevice->ops->set_waterMark(mDevice, enable, str);
		}
		return INVALID_OPERATION; 
	}
    ...
    status_t setUvcGadgetMode(int value)
	{
		...
		if(mDevice->ops->set_uvcGadgetMode) {
			return mDevice->ops->set_uvcGadgetMode(mDevice, value);
		}
		return INVALID_OPERATION; 
	}
    ...
    status_t adasSetGsensorData(int val0, float val1, float val2)
	{
		...
		if(mDevice->ops->set_adasGsensorData) {
			return mDevice->ops->set_adasGsensorData(mDevice, val0, val1, val2);
		}
		return INVALID_OPERATION; 
	}
...
private:
    ...
    static int __data_cb_timestamp(nsecs_t timestamp, int32_t msg_type,
                             ...
                             int bufIdx, void *user)
    {
        ...
        return __this->mDataCbTimestamp(timestamp, msg_type, mem->mBuffers[index], bufIdx, __this->mCbUser);
    }
    ...
    static unsigned int __to_anw(void *user)
    {
        CameraHardwareInterface *__this =
                reinterpret_cast<CameraHardwareInterface *>(user);
        //return __this->mPreviewWindow.get();
        return __this->mPreviewLayerId;
    }
...
//        }
//        return rc;
        ALOGE("(f:%s, l:%d) think other method!", __FUNCTION__, __LINE__);
        return -1;
    }
...
//        return a->query(a, NATIVE_WINDOW_MIN_UNDEQUEUED_BUFFERS, count);
    ...
    static int __awextend_perform(struct preview_stream_ops* w, int cmd0, int cmd1, int value)
    {
        ALOGE("(f:%s, l:%d) need implement this function!", __FUNCTION__, __LINE__);
        switch(cmd0)
        {
//            case PREVIEW_STREAM_OPS_CMD_HWLayerRender:
//            {
//                break;
//            }
            default:
            {
                ALOGW("(f:%s, l:%d) unknown cmd[0x%x]!", __FUNCTION__, __LINE__, cmd0);
                break;
            }
        }
        return -1;
    ...
    void initHalPreviewWindow()
    {
        ...
        mHalPreviewWindow.nw.perform = __awextend_perform;
    }
