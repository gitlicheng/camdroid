...
public:
    ...
    void getSubChannelSize(int *width, int *height) const;
    ...
    void getSupportedPreviewFrameRates(Vector<int> &frameRates) const;
	...
	static const char KEY_CID_CONTRAST[];
	static const char KEY_CID_BRIGHTNESS[];
	static const char KEY_CID_SATURATION[];
	static const char KEY_CID_HUE[];
	static const char KEY_CID_SHARPNESS[];
    static const char KEY_CID_HFLIP[];
    static const char KEY_CID_VFLIP[];    
    static const char KEY_CID_EFFECT[];
    static const char KEY_CID_POWER_LINE_FREQUENCY[];
	static const char KEY_CAPTURE_OUT_FORMAT[];
    static const char KEY_SUBCHANNEL_SIZE[];

    static const char KEY_AWEXTEND_PREVIEW_FLIP[];
    static const char KEY_AWEXTEND_PICTURE_MODE[];
    static const char KEY_AWEXTEND_PICTURE_SIZE_MODE[];
    //values for KEY_AWEXTEND_PREVIEW_FLIP
    struct AWEXTEND_PREVIEW_FLIP
    {
        static const int NoFlip = 0;
        static const int LeftRightFlip = 1;
        static const int TopBottomFlip = 2;
    private:
        AWEXTEND_PREVIEW_FLIP() {};
    };
    //values for KEY_AWEXTEND_PICTURE_MODE
    static const char AWEXTEND_PICTURE_MODE_NORMAL[];
    static const char AWEXTEND_PICTURE_MODE_FAST[];
    static const char AWEXTEND_PICTURE_MODE_CONTINUOUS[];
    static const char AWEXTEND_PICTURE_MODE_CONTINUOUS_FAST[];

    // set takePicture number for continuous picture mode
    static const char KEY_AWEXTEND_CONTINUOUS_PICTURE_NUMBER[];
    // set continuous picture interval
    static const char KEY_AWEXTEND_CONTINUOUS_PICTURE_INTERVAL[];
    static const char KEY_AWEXTEND_CONTINUOUS_PICTURE_PATH[];
    static const char KEY_AWEXTEND_SNAP_PATH[];

    static const char KEY_AWEXTEND_CAMERA_HAL_VERSION[];

    //values for KEY_AWEXTEND_PICTURE_SIZE_MODE
    struct AWEXTEND_PICTURE_SIZE_MODE
    {
        static const int KeepSourceSize = 0;
        static const int UseParameterPictureSize = 1;
    private:
        AWEXTEND_PICTURE_SIZE_MODE() {};
    };

    static const char KEY_AWEXTEND_VIDEO_CAPTURE_BUFFER_NUM[];

    static const char KEY_AWEXTEND_PREVIEW_ROTATION[];
