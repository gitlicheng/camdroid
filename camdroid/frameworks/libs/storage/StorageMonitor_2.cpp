...
int StorageMonitor::updateLoop(void) {
	...
	mStatus = -1;
	processStr(snd_buf);
	ALOGD("----------------------- mListener notify: mStatus = %d, mPath = %s \n", mStatus, mPath);
	if(mListener != NULL) {
		switch(mStatus) {
			...
			case STORAGE_STATUS_IDLE: {
				ALOGD("---0000------mShared=%d, mSharing = %d, mFormated=%d, mFormating = %d, mInserted = %d", mShared, mSharing, mFormated, mFormating, mInserted);
				if(mFormated == true) {
					if(mFormating == false) {
						mFormating = true;
    					...
    					if(mShared == true) {
    						...
    						if(mInserted == false) {
                                ...
                                if(STORAGE_STATUS_SHARED == oStatus || STORAGE_STATUS_PENDING == oStatus){
                                    ...
                                    ALOGE("22222oStatus=%d",oStatus);//不会走这里
                                    write(mConnectFD, CMD_SHARE, sizeof(CMD_SHARE));
                                }else{
                                    ALOGE("333333oStatus=%d",oStatus);
                                }
    						}						
    					}                    
