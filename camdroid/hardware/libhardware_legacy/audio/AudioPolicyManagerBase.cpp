                                    ...
                                    AudioSystem::audio_in_acoustics acoustics)
{
    ...
    if (!mInputs.isEmpty()) {
        for (size_t i = 0; i < mInputs.size(); i++) {
            ...
            if (inputDescriptor->mProfile == profile) {
                ALOGI("input(%d) has the same profile(%p).", mInputs.keyAt(i), profile);
...
status_t AudioPolicyManagerBase::startInput(audio_io_handle_t input)
{
...
#endif //AUDIO_POLICY_TEST
    {
        ...
        if (getActiveInput() != 0) {
            ...
            //return INVALID_OPERATION;
            if (input == getActiveInput()) {
                ALOGI("start the same input %d.", input);
                inputDesc->mRefCount++;
                return NO_ERROR;
            } else {
                ALOGW("startInput() input %d failed: other input already started", input);
...
void AudioPolicyManagerBase::releaseInput(audio_io_handle_t input)
{
    ...
    AudioInputDescriptor *inputDesc = mInputs.valueAt(index);
    if (inputDesc->mRefCount > 0) {
        ALOGI("input ref count is %d", inputDesc->mRefCount);
        return;
    }
    /*added by ywl for multi recorder --end*/
    mpClientInterface->closeInput(input);
