...
typedef int (*camera_data_timestamp_callback)(int64_t timestamp,
        int32_t msg_type,
        const camera_memory_t *data, unsigned int index, int bufIdx,
        void *user);
...
typedef struct camera_device_ops {
    ...
    */
	...
	int (*get_v4l2_query_ctrl)(struct camera_device *,
		struct v4l2_queryctrl *, int id);

	int (*get_input_source)(struct camera_device *, int *);

	int (*get_tvin_system)(struct camera_device *, int *);

    int (*set_waterMark)(struct camera_device *, int, const char *);

    int (*set_uvcGadgetMode)(struct camera_device *, int value);

    int (*set_adasGsensorData)(struct camera_device *, int val0, float val1, float val2);
