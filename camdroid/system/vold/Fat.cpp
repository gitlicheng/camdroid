...
int Fat::doMount(const char *fsPath, const char *mountPoint,
    ...
    if (value[0] == '1') {
        ALOGW("line=%d,The SD card is world-writable because the"
            " 'persist.sampling_profiler' system property is set to '1'.",__LINE__);
        permMask = 0;
    }
    ...
    ALOGD("line=%d,mount ret=%d",__LINE__,rc);
    if (rc && errno == EROFS) {
        ALOGE("line=%d appears to be a read only filesystem - retrying mount RO",__LINE__,fsPath);
        //flags |= MS_RDONLY;
        rc = mount(fsPath, mountPoint, "vfat", flags, mountData);
    }
