...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
    ...
    ALOGD("CommandListener::VolumeCmd::runCommand");
    if (argc < 2) {
    ...
    if (!strcmp(argv[1], "list")) {
        ...
        ALOGD("3333argv[1]=%s,argc=%d",argv[1],argc);
        if (argc != 3) {
        ...
        ALOGD(" argv[2]=%s,force=%d,revert=%d",argv[2],force,revert);
        rc = vm->unmountVolume(argv[2], force, revert);
    } else if (!strcmp(argv[1], "format")) {
        if (argc != 3) {
            ALOGD(" 11111argc=%d",argc);
        ...
        ALOGE(" argv[1]=%s,argc=%d",argv[1],argc);
        if (argc != 4) {
            cli->sendMsg(ResponseCode::CommandSyntaxError,
                    "Usage: volume unshare <path> <method>", false);
            return 0;
        }
        ALOGE(" argv[2]=%s,argv[3]=%s",argv[2],argv[3]);
        rc = vm->unshareVolume(argv[2], argv[3]);
        ALOGD(" vm->unshareVolume rc=%d",rc);
    ...
    if (!rc) {
        ...
        int erno = errno;
        ALOGE("22222VolumeCmd::runCommand volume operation failed");
