        ...
        if (!strncmp(dp, *it, strlen(*it))) {
            ...
            if (action == NetlinkEvent::NlActionAdd) {
                ...
                if (!strcmp(devtype, "disk")) {
                    ALOGD("line=%d,devtype=%s",__LINE__,devtype);
                ...
                if (!strcmp(devtype, "disk")) {
                   ...
                   ALOGW("Ignoring non add/remove/change event");
            }
...
void DirectVolume::handleDiskAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    if (tmp) {
        ...
        ALOGW("Kernel block uevent missing 'NPARTS'");
        mDiskNumParts = 1;
    }
    ALOGD("line=%d,handleDiskAdded mDiskNumParts=%d",__LINE__,mDiskNumParts);
     mPartsEventCnt = 0;
    ...
    if (mDiskNumParts == 0) {
        ...
        ALOGD("Dv::diskIns - No partitions - good to go son!");
#endif
        setState(Volume::State_Idle);
		//ALOGD("Dv::diskIns - No partitions - good to go son!");
        int flag = getShareFlags();
        if(flag != 1){
            ALOGD("line=%d,handleDiskAdded call mountVol",__LINE__);
            mountVol();
        }
    } else {
#ifdef PARTITION_DEBUG
        //ALOGD("Dv::diskIns - waiting for %d partitions (mask 0x%x)",
...
void DirectVolume::handlePartitionAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    ALOGD("handlePartitionAdded devpath=%s,mPartsEventCnt=%d",devpath,mPartsEventCnt);     
    if(mPartsEventCnt > mDiskNumParts){
        ALOGW("Partition event is to much, mPartsEventCnt=%d, mDiskNumParts=%d\n", mPartsEventCnt, mDiskNumParts);
        mPartsEventCnt = mDiskNumParts;
    }else{
        mPartsEventCnt++;
    }

    ALOGD("handlePartitionAdded mDiskNumParts=%d,mPartsEventCnt=%d",mDiskNumParts,mPartsEventCnt);
    if (tmp) {
        part_num = atoi(tmp);
    } else {
        ALOGW("Kernel block uevent missing 'PARTN'");
        part_num = 1;
    }

    if (part_num > MAX_PARTITIONS || part_num < 1) {
        ALOGE("--------Invalid 'PARTN' value");
        return;
    }
    ...
    if (major != mDiskMajor) {
        ALOGE("Partition '%s' has a different major than its disk!", devpath);
        return;
    }
#ifdef PARTITION_DEBUG
    ALOGD("Dv:partAdd: part_num = %d, minor = %d\n", part_num, minor);
#endif
    if (part_num >= MAX_PARTITIONS) {
        ALOGE("Dv:partAdd: ignoring part_num = %d (max: %d)\n", part_num, MAX_PARTITIONS-1);
    } else {
        ...
        if (getState() != Volume::State_Formatting) {
           ...
           // if (mRetryMount == true) {
            //    mRetryMount = false;
                int flag = getShareFlags();
                if(flag != 1){
                    ALOGD("line-%d,handlePartitionAdded",__LINE__);
                    mountVol();
                }
                
           // }else{
            //    ALOGD("handlePartitionAdded mRetryMount=true");
          //  }
        }
    } else {
        ALOGD(" pending mask now = 0x%x", mPendingPartMap);
#ifdef PARTITION_DEBUG
        ALOGD("Dv:partAdd: pending mask now = 0x%x", mPendingPartMap);
#endif
    }
}
...
void DirectVolume::handleDiskChanged(const char *devpath, NetlinkEvent *evt) {
    ...
    ALOGI("Volume %s disk has changed", getLabel());
    const char *tmp = evt->findParam("NPARTS");
    if (tmp) {
        mDiskNumParts = atoi(tmp);
    } else {
...
void DirectVolume::handlePartitionChanged(const char *devpath, NetlinkEvent *evt) {
    int major = atoi(evt->findParam("MAJOR"));
    int minor = atoi(evt->findParam("MINOR"));
    ALOGD("Volume %s %s partition %d:%d changed\n", getLabel(), getMountpoint(), major, minor);
}
...
void DirectVolume::handleDiskRemoved(const char *devpath, NetlinkEvent *evt) {
    ...
    //ALOGD("Volume %s %s disk %d:%d removed\n", getLabel(), getMountpoint(), major, minor);
   // snprintf(msg, sizeof(msg), "Volume %s %s disk removed (%d:%d)",
   //          getLabel(), getMountpoint(), major, minor);
   
   snprintf(msg, sizeof(msg),
		 "Volume %s %s state changed from %d (%s) to %d (%s)", getLabel(),
		 getMountpoint(), getState(), "somestate", Volume::State_Remove,
		 "Remove");
     mVm->getBroadcaster()->sendBroadcast(ResponseCode::VolumeDiskRemoved,
                                             msg, false);
     //usleep(100 * 1000);                                        
    /* 2011-4-30 force unmount fs */
   // ALOGD("DirectVolume : handleDiskRemoved : unmountVol");
...
void DirectVolume::handlePartitionRemoved(const char *devpath, NetlinkEvent *evt) {
    ...
    ALOGD("Volume %s %s partition %d:%d removed\n", getLabel(), getMountpoint(), major, minor);
	...
	if (mVm->cleanupAsec(this, true)) {
            ALOGE("Failed to cleanup ASEC - unmount will probably fail!");
        }

        if (Volume::unmountVol(true, false)) {
            ALOGE("Failed to unmount volume on bad removal (%s)", 
                 strerror(errno));
            // XXX: At this point we're screwed for now
        } else {
            ALOGD("Crisis averted");
        }
        ...
        if (mVm->unshareVolume(getLabel(), "ums")) {
            ALOGE("Failed to unshare volume on bad removal (%s)",
                strerror(errno));
        } else {
            ALOGD("Crisis averted");
        }
...
int DirectVolume::getDeviceNodes(dev_t *devs, int max) {
    ALOGD("getDeviceNodes mPartIdx=%d,mDiskNumParts=%d",mPartIdx,mDiskNumParts);
    if (mPartIdx == -1) {
...
int DirectVolume::updateDeviceInfo(char *new_path, int new_major, int new_minor)
{
    ...
    if (mPartIdx == -1) {
        ALOGE("Can only change device info on a partition\n");
        return -1;
    }
     ...
     */
    if (mPaths->size() != 1) {
        ALOGE("Cannot change path if there are more than one for a volume\n");
        return -1;
    }
