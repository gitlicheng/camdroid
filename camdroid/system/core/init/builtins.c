...
int do_mount(int nargs, char **args)
{
	...
	//	listdir("/dev");
        ...
        INFO("out of loopback devices source = %s\n",source);
        INFO("out of loopback devices target = %s\n",target);
        INFO("out of loopback devices system = %s\n",system);
        INFO("out of loopback devices options = %s\n",options);
	//	listdir("/dev");
        if (mount(source, target, system, flags, options) < 0) {
            listdir("/dev/block");
            INFO("do_mount errno=%d\n",errno);
            return -1;
        }
    }
