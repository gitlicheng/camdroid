...
#include "include/listbox_impl.h"

#ifdef LOG_TAG
#undef LOG_TAG
#endif
#define LOG_TAG "MenuList.cpp"
#include "debug.h"
...
static unsigned char HILIGHT_LEFT = 0;		/* pixels to the left of the selected hilighted item */
...
static void lstCalcParams (const RECT* rcClient, PLISTBOXDATA pData)
{
	...
	db_msg("itemVisibles is %d\n", pData->itemVisibles);
}

/*
 * |                                  |
 * |----------------------------------|
 * |     ▅▅▅▅▅▅▅▅▅▅▅▅▅▅     |   temp_rect_left -> ▅▅▅▅▅▅▅  <- temp_rect_right
 * |----------------------------------|
 * |   ▅▅▅▅▅▅▅▅▅▅▅▅▅▅       |   hilited item:▅▅▅▅▅▅▅
 * |----------------------------------|
 */
	...
	for (i = 0; plbi && i < (pData->itemVisibles + 1); i++) {
		...
		if (plbi->dwFlags & LBIF_SELECTED) {
			...
			if (!(pWin->dwExStyle & WS_EX_TRANSPARENT))
				pWin->we_rdr->draw_normal_item (hWnd, hdc, &rect, gp_normal_bgc);
		}
		...
		/* 1. ************ 使用了LBS_USEBITMAP 而且cmFlag包含了IMGFLAG_IMAGE则需要在开头的地方显示图标 *************/
		if(plbi->imageFlag & LBIF_IMAGE && plbi->dwImage != 0) {
			...
			//value2 = int(pData->itemHeight*0.75);
			//value1 = value2*0.4;
...
int MenuListCallback(HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_CREATE:
		{
			...
			gp_normal_bgc = menuListAttr->normalBgc;	//UI@normal_background
			gp_normal_fgc = menuListAttr->normalFgc;	//UI@normal_foreground
			...
			gp_selected_stringc = menuListAttr->selectedStringc;	//UI@string_selected_color
			gp_selected_valuec = menuListAttr->selectedValuec;		//UI@value_selected_color

			/****** scoll bar use the mainc_3dbox(WE_MAINC_THREED_BODY) element color, the element is RGBA format **********/
			
			/*********  set the scroll bar width ************/
			SetWindowElementAttr(hwnd, WE_METRICS_SCROLLBAR, 4);	//UI@scroll_bar_height
	...
	case LB_INSERTSTRING:
		/*如果指定了LBS_CHECKBOX或者LBS_USEICON属性，那么
		 * lParam pass the PLISTBOXITEMINFO, 否则lParam就是传递了string的指针 */
		{
			char* string = NULL;
			...
			if (pmlii) {
				...
				/* end of add and changed by caodao to support display text*/
			...
			if (message == LB_ADDSTRING)
				...
				/*message == LB_INSERTSTRING, wParam表示index索引值*/
	...
	case MSG_KEYDOWN:
		{
			...
			db_msg("newTop is %d\n", newTop);
			db_msg("newSel is %d\n", newSel);
			db_msg("item count is %d\n", pData->itemCount);
			db_msg("item visible is %d\n", pData->itemVisibles);
			db_msg("item bottom is %d\n", ITEM_BOTTOM(pData));
#endif
	...
	case LB_MULTIADDITEM:
		{
			...
			/*********** determine the lParam is a single string or the MENULISTITEMINFO ************/
			if (dwStyle & LBS_CHECKBOX || dwStyle & LBS_USEBITMAP || dwStyle & LBS_HAVE_VALUE) {
				pmlii = (PMENULISTITEMINFO)lParam;
				if (!pmlii)
					return LB_ERR;
