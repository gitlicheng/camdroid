	      ...
	      int format, int grabmethod)
{
	...
	if (vd == NULL || device == NULL)
 	...
 	vd->toggleAvi = 0;
  	...
  	if (init_v4l2 (vd) < 0) {
    		LOGE("Init v4L2 failed !! exit fatal \n");
    		goto error;;
  	}
  	...
  	case V4L2_PIX_FMT_YUYV:
    		...
    		break;
  	default:
    		LOGE("should never arrive exit fatal !!\n");
    		goto error;
    		break;
  	}
  	if (!vd->framebuffer)
...
static int init_v4l2 (struct vdIn *vd)
{
  	...
  	if ((vd->fd = open (vd->videodevice, O_RDWR)) == -1) {
    		LOGE ("ERROR opening V4L interface \n");
    		exit (1);
  	}
  	...
  	if ((vd->cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) == 0) {
    	     	LOGE("Error opening device %s: video capture not supported.\n",
	     		vd->videodevice);
    		goto fatal;;
  	}
  	if (vd->grabmethod) {
  	...
  	if (ret < 0) {
    		LOGE("Unable to allocate buffers: %d.\n", errno);
    		goto fatal;
  	}
  	/* map the buffers */
  	for (i = 0; i < NB_BUFFER; i++) {
    		...
    		if (ret < 0) {
      			LOGE("Unable to query buffer (%d).\n", errno);
      			goto fatal;
    		}
    		if (debug)
      			LOGE("length: %u offset: %u\n", vd->buf.length,vd->buf.m.offset);
    		vd->mem[i] = mmap (0 /* start anywhere */ ,
		vd->buf.length, PROT_READ, MAP_SHARED, vd->fd,vd->buf.m.offset);

		if (vd->mem[i] == MAP_FAILED) {
      			LOGE("Unable to map buffer (%d)\n", errno);
      			goto fatal;
    		}
    		if (debug)
      			LOGE("Buffer mapped at address %p.\n", vd->mem[i]);
  	}
  	/* Queue the buffers. */
  	for (i = 0; i < NB_BUFFER; ++i) {
    		memset (&vd->buf, 0, sizeof (struct v4l2_buffer));
...
#define HEADERFRAME1 0xaf
  	...
  	if (ret < 0) {
    		LOGE("Unable to dequeue buffer (%d).\n", errno);
    		goto err;
  	}
	...
	case V4L2_PIX_FMT_YUYV:
  	...
  	ret = ioctl (vd->fd, VIDIOC_QBUF, &vd->buf);
  	if (ret < 0) {
    		LOGE("Unable to requeue buffer (%d).\n", errno);
    		goto err;
  	}
...
static int isv4l2Control (int fd, int control, struct v4l2_queryctrl *queryctrl)
{
  	int err = 0;
  	queryctrl->id = control;
  	if ((err = ioctl (fd, VIDIOC_QUERYCTRL, queryctrl)) < 0) {
    		LOGE("ioctl querycontrol error %d \n", errno);
...
int v4l2ToggleControl (int fd, int control)
{
  	...
  	control_s.value = !current;
  	if ((err = ioctl (fd, VIDIOC_S_CTRL, &control_s)) < 0) {
    		LOGE("ioctl toggle control error\n");
    		return -1;
  	}
  	return control_s.value;
}
...
int v4l2ResetControl (int fd, int control)
{
  	...
  	if ((err = ioctl (fd, VIDIOC_S_CTRL, &control_s)) < 0) {
    		LOGE("ioctl reset control error the cmd is 0x%08x\n",VIDIOC_S_CTRL);
    		return -1;
  	}
...
int v4l2ResetPanTilt (int fd, int pantilt)
{
  	...
  	control_s.value = val;
  	if ((err = ioctl (fd, VIDIOC_S_CTRL, &control_s)) < 0) {
    		LOGE("ioctl reset Pan control error\n");
    	return -1;
  	}
...
int v4L2UpDownPan (int fd, short inc)
{
  	...
  	if ((err = ioctl (fd, VIDIOC_S_CTRL, &control_s)) < 0) {
    		LOGE("ioctl pan updown control error\n");
    	return -1;
  	}
  	return 0;
}

int v4L2UpDownTilt (int fd, short inc)
{
  	int control = V4L2_CID_PANTILT_RELATIVE;
  	struct v4l2_control control_s;
  	struct v4l2_queryctrl queryctrl;
 	int err;
  	...
  	control_s.value = pan.value;
  	if ((err = ioctl (fd, VIDIOC_S_CTRL, &control_s)) < 0) {
    		LOGE("ioctl tiltupdown control error\n");
    		return -1;
  	}
  	return 0;
}
