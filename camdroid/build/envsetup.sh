...
function setpaths()
{
	...
	export CAMLINUX_PRODUCT_FEATURE=$(get_build_var CAMLINUX_PRODUCT_FEATURE)
	echo "CAMLINUX_PRODUCT_FEATURE $CAMLINUX_PRODUCT_FEATURE"
...
function doc_link()
{
    local doc="AWDocument"
    if [ -d "$doc" ]; then
	rm $doc
    fi
    if [ -d "device/softwinner/$tdevice/$doc" ]; then
	ln -s device/softwinner/$tdevice/$doc $doc
    fi
}

function get_key()
{
    local section=$(echo $1 | cut -d '.' -f 1)
    local key=$(echo $1 | cut -d '.' -f 2)
    sed -n "/\[$section\]/,/\[.*\]/{
     /^\[.*\]/d
     /^[ \t]*$/d
     /^$/d
...
function list_dir()
{
    ...
    local dir=$(ls -l $1 |awk '/^d/ {print $NF}')
    for i in $dir
    do
	CUSTOM_DIRS=(${CUSTOM_DIRS[@]} $i)
    done
}
...
function check_custom_config()
{
    unset CUSTOM_DIRS
    local answer
    local custom_dir="device/softwinner/$tdevice/custom"
    list_dir $custom_dir
    if ((${#CUSTOM_DIRS[@]} < 1 )); then
	return 1
    fi
    local i=1
    local choice
    for choice in ${CUSTOM_DIRS[@]}
    do
	echo -en "\033[36m\t$i. $choice : \033[0m"
        custom_dir="device/softwinner/$tdevice/custom/$choice"
        config_file="$custom_dir/xconfig.ini"
	if [ -e $config_file ]; then
	    name=$(get_key "product.name" $config_file)
	    desc=$(get_key "product.desc" $config_file)
...
#	    echo -e "\033[36m\t\t$desc \033[0m"
    ...
    echo -n "Choose the custom config number: "
    read answer
    local selection=
    if [ -z "$answer" ]
    then
        selection=
    elif (echo -n $answer | grep -q -e "^[1-9][1-9]*$")
    then
        if [ $answer -le ${#CUSTOM_DIRS[@]} ]
        then
            selection=${CUSTOM_DIRS[$(($answer-1))]}
        fi
    fi
    ...
    then
        echo
        echo "Use the default config."
        return 1
    fi
    echo -n Use
    echo -ne "\033[31m $selection \033[0m"
    echo "config."
    local select_dir="device/softwinner/$tdevice/custom/$selection"
    local camdroid_config="$select_dir/camdroid"
    local linux_config="$select_dir/lichee/*_defconfig*"
    local pack_config="$select_dir/lichee/pack_config/*"

    if [ -d "$select_dir" ]; then
	cp -vrf $camdroid_config/* device/softwinner/$tdevice/
        cp -vrf $linux_config $LICHEE_KERN_DIR/arch/arm/configs/
	rm -v $LICHEE_KERN_DIR/.config
	cp -vrf $pack_config $LICHEE_TOOLS_DIR/pack/chips/$LICHEE_CHIP/configs/$tdevice/
    fi
}
...
function mm()
{
    ...
    if [ -f build/core/envsetup.mk -a -f Makefile ]; then
        ...
        # Find the closest CamLinux.mk file.
        T=$(gettop)
...
function mmm()
{
    ...
    if [ "$T" ]; then
        ...
        for DIR in $DIRS ; do
            ...
            if [ -f $DIR/CamLinux.mk ]; then
                TO_CHOP=`(cd -P -- $T && pwd -P) | wc -c | tr -d ' '`
                TO_CHOP=`expr $TO_CHOP + 1`
                START=`PWD= /bin/pwd`
                MFILE=`echo $START | cut -c${TO_CHOP}-`
                if [ "$MFILE" = "" ] ; then
                    MFILE=$DIR/CamLinux.mk
                else
                    MFILE=$MFILE/$DIR/CamLinux.mk
                fi
