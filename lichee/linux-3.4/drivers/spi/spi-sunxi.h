...
#define SPI_MODULE_FREQUENCY 	100000000 	/* spi module clock frequency, can be modified(no greater then pll clock) */
...
#define SUNXI_SPI_MEM_BASE		SUNXI_SPI0_PBASE
#define SUNXI_SPI_MEM_RANGE		0x1000
#define SUNXI_SPI_MEM_START(ch)	(SUNXI_SPI_MEM_BASE + ch * SUNXI_SPI_MEM_RANGE)
#define SUNXI_SPI_MEM_END(ch)	(SUNXI_SPI_MEM_START(ch) + SUNXI_SPI_MEM_RANGE - 1)

/* The IRQ number. */

#ifdef CONFIG_EVB_PLATFORM
#define SUNXI_SPI_IRQ(ch)	    (SUNXI_IRQ_SPI0 + ch)
#else
#define SUNXI_SPI_IRQ(ch)	    SUNXI_IRQ_SPI0 /* In FPGA, only one IRQ is available. */
#endif
