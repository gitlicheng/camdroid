...
#define rf_pm_msg(...)    do {printk("[wifi module_pm]: "__VA_ARGS__);} while(0)
...
int rf_module_power(int onoff)
{
	...
	if (onoff) {
		...
		for(i = 0; wifi_ldo[i] != NULL; i++){
			rf_pm_msg("regulator vol[%d]=%d.\n",i,mod_info.power[i].vol);
			if(mod_info.power[i].vol > 0) {
				rf_pm_msg("mod_info.power[i].vol > 0\n");
...
static int get_module_res(void)
{
	...
	for(i = 0; i < 4; i++) {
		...
		if(SCIRPT_ITEM_VALUE_TYPE_STR != type) {
			rf_pm_msg("step [%d] failed to fetch %s\n",i,power_table[i].power_name);
			break;
		} else {
			if(!strcmp(val.str, "")) {
				break;
