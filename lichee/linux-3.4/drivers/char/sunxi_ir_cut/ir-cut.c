...
static  int g_iradc_reg_select  = 0;
...
static int ir_cut_fetch_sysconfig_para(void)
{
	...
	if(SCIRPT_ITEM_VALUE_TYPE_INT != ir_cut_sys_script_get_item("ir_cut_para", "iradc_reg_select", &g_iradc_reg_select, 1)){
	      printk("ircut: script_parser_fetch iradc_reg_select fail. \n");
	    g_iradc_reg_select = 1;
	}
...
{
	int lradc_data = 0xff;
	if ( 0 == g_iradc_reg_select ) {
    	lradc_data = read32((LRADC_DATA0_T+LRADC_BASE));
    } else {
    	lradc_data = read32((LRADC_DATA1_T+LRADC_BASE));
    }
//	printk("ir_cut_read[%d]..day_night = %d, lradc_data = %d,g_iradc_reg_select=%d\n", __LINE__, ir_cut_devp->day_night, lradc_data,g_iradc_reg_select);
    if(1 == ir_cut_devp->day_night && lradc_data < 26) {
		__inf("step[19]lradc_data1 = [%d]\n", lradc_data1);
		ir_cut_devp->day_night = 0 ; /* ir cut vol <0.6 V  ,day_night = 0,°×Ìì;day_night = 1,ºÚÒ¹ */
	}
	else if(0 == ir_cut_devp->day_night && lradc_data > 52) {
...
//	struct ir_cut_dev *dev = filp->private_data; 
	...
	if (g_iradc_reg_select == 0) {
		if(buf_value == 1) {
			ir_ctl_set_edge(1);
			led_on(0);
//	        printk("=>night   riseedge \n");
		}
		else if (buf_value == 0) 
		{	
	        ir_ctl_set_edge(0);
