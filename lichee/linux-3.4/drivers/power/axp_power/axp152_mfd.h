...
static int __devinit axp152_init_chip(struct axp_dev *chip)
{
	...
	int err;
	/*read chip id*/
	err =  __axp_read(NULL, chip->client, POWER15_IC_TYPE, &chip_id, false);
	if (err) {
	...
	err =  __axp_writes(NULL, chip->client, POWER15_INTEN1, 11, v, false);
	if (err) {
    ...
    /* enable dcdc2 dvm */
	err =  __axp_read(NULL, chip->client, 0x25, &dcdc2_ctl, false);
	if(err){
		printk(KERN_ERR "[AXP15-MFD] try to read reg[25H] failed!\n");
		return err;
	}
	dcdc2_ctl |= (1<<2);
	err = __axp_write(NULL, chip->client, 0x25, dcdc2_ctl, false);
	if(err){
...
static int axp152_disable_irqs(struct axp_dev *chip, uint64_t irqs)
{
	...
	ret =  __axp_writes(NULL, chip->client, POWER15_INTEN1, 5, v, false);
#ifdef CONFIG_ARCH_SUN7I
...
static int axp152_read_irqs(struct axp_dev *chip, uint64_t *irqs)
{
	uint8_t v[3] = {0, 0, 0};
	int ret;
	ret =  __axp_reads(NULL, chip->client, POWER15_INTSTS1, 3, v, false);
#ifdef CONFIG_ARCH_SUN7I
	writel(0x01,NMI_IRG_PENDING_REG);
#endif
	if (ret < 0)
