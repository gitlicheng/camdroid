...
int axp20_fetch_sysconfig_para(char * pmu_type, struct axp20_config_info *axp_config)
{
	...
	if (axp_config->pmu_used) {
		...
		ret = axp_script_parser_fetch(pmu_type, "pmu_battery_warning1", &axp_config->pmu_battery_warning1, sizeof(int));
		if (ret)
		{
			printk("axp driver uning configuration failed(%d)\n", __LINE__);
			axp_config->pmu_battery_warning1 = 0;
		}
		ret = axp_script_parser_fetch(pmu_type, "pmu_battery_warning2", &axp_config->pmu_battery_warning2, sizeof(int));
		if (ret)
		{
			printk("axp driver uning configuration failed(%d)\n", __LINE__);
			axp_config->pmu_battery_warning2 = 0;
		}
		ret = axp_script_parser_fetch(pmu_type, "pmu_chg_stop_temp", &axp_config->pmu_chg_stop_temp, sizeof(int));
		if (ret)
		{
			printk("axp driver uning configuration failed(%d)\n", __LINE__);
		...
		ret = axp_script_parser_fetch(pmu_type, "pmu_chg_restart_temp", &axp_config->pmu_chg_restart_temp, sizeof(int));
		if (ret)
		{
			printk("axp driver uning configuration failed(%d)\n", __LINE__);
