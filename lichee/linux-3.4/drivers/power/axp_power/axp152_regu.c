...
printk("%s,%d\n", __func__, __LINE__);
	...
	case AXP152_ID_LDO0:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_ldo0_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDO2:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_aldo12_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDO3:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_aldo12_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDO4:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDO5:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_DCDC1:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_dcdc1_voltage(rdev, uV, uV, &select);
	case AXP152_ID_DCDC2:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_DCDC3:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_DCDC4:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDO1:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	case AXP152_ID_LDOIO0:
		printk("%s,line:%d\n", __func__, __LINE__);
		return axp_set_voltage(rdev, uV, uV, &select);
	default:
		return -EINVAL;
...
static int axp_ldoio0_enable(struct regulator_dev *rdev)
{
	...
	struct device *axp_dev = to_axp_dev(rdev);
	 ...
	 return axp_update(axp_dev, info->enable_reg,0x02,0x07);
}
...
static int axp_ldoio0_disable(struct regulator_dev *rdev)
{
	...
	return axp_update(axp_dev, info->enable_reg,0x07,0x07);
}
...
#define AXP15_LDO(_id, min, max, step, vreg, shift, nbits, ereg, ebit)	\
	AXP_LDO(AXP152, _id, min, max, step, vreg, shift, nbits, ereg, ebit)

#define AXP15_BUCK(_id, min, max, step, vreg, shift, nbits, ereg, ebit)	\
	AXP_BUCK(AXP152, _id, min, max, step, vreg, shift, nbits, ereg, ebit)

#define AXP15_DCDC(_id, min, max, step, vreg, shift, nbits, ereg, ebit)	\
	AXP_DCDC(AXP152, _id, min, max, step, vreg, shift, nbits, ereg, ebit)
...
static struct axp_regulator_info axp_regulator_info[] = {
	...
	AXP15_LDO(	4,	700,		3500,		100,	DIGITAL1,	0,	5,	DIGITAL1EN,	1),//ldo2 for DigtalLDO1
	AXP15_LDO(	5,	700,		3500,		100,	DIGITAL2,	0,	5,	DIGITAL2EN,	0),//ldo3 for DigtalLDO2
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	uint8_t val;
	ret = axp_read(axp_dev, AXP152_BUCKMODE, &val);
	if (ret)
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(info->desc.id == AXP152_ID_DCDC1){
		if(val)
			axp_set_bits(axp_dev, AXP152_BUCKMODE,0x08);
		else
			axp_clr_bits(axp_dev, AXP152_BUCKMODE,0x08);
	}
	if(info->desc.id == AXP152_ID_DCDC2){
		if(val)
			axp_set_bits(axp_dev, AXP152_BUCKMODE,0x04);
		else
			axp_clr_bits(axp_dev, AXP152_BUCKMODE,0x04);
	}
	if(info->desc.id == AXP152_ID_DCDC3){
		if(val)
			axp_set_bits(axp_dev, AXP152_BUCKMODE,0x02);
		else
			axp_clr_bits(axp_dev, AXP152_BUCKMODE,0x02);
	}
	else if(info->desc.id == AXP152_ID_DCDC4){
		if(val)
			axp_set_bits(axp_dev, AXP152_BUCKMODE,0x01);
		else
			axp_clr_bits(axp_dev, AXP152_BUCKMODE,0x01);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	uint8_t val;
	ret = axp_read(axp_dev, AXP152_BUCKFREQ, &val);
	if (ret)
		return ret;
	ret = val & 0x0F;
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	axp_read(axp_dev, AXP152_BUCKFREQ, &tmp);
	tmp &= 0xF0;
	val |= tmp;
	axp_write(axp_dev, AXP152_BUCKFREQ, val);
	return count;
}
...
static int __devinit axp_regulator_probe(struct platform_device *pdev)
{
	...
	if (ri->desc.id == AXP152_ID_LDO4 || ri->desc.id == AXP152_ID_LDO5 \
							||ri->desc.id == AXP152_ID_DCDC2 \
							||ri->desc.id == AXP152_ID_DCDC3 \
							||ri->desc.id == AXP152_ID_DCDC4 \
							||ri->desc.id == AXP152_ID_LDO1 ){
			ri->desc.ops = &axp15_ops;
			printk("Register AXP15_OPS sucess!\n");
	}

	if(ri->desc.id == AXP152_ID_LDOIO0)
	{
		ri->desc.ops = &axp15_ldoio0_ops;
		printk("Register AXP15_ldoio0_OPS finish!\n");
	}
	if(ri->desc.id == AXP152_ID_LDO0)
	{
		ri->desc.ops = &axp15_ldo0_ops;
		printk("Register AXP15_ldo0_OPS finish!\n");
	}

	if(ri->desc.id == AXP152_ID_LDO3 || ri->desc.id == AXP152_ID_LDO2)
	{
		ri->desc.ops = &axp15_aldo12_ops;
		printk("Register AXP15_aldo12_OPS finish!\n");
		}

	if(ri->desc.id == AXP152_ID_DCDC1)
		{
	...
	if(ri->desc.id == AXP152_ID_DCDC1 ||ri->desc.id == AXP152_ID_DCDC2 \
		||ri->desc.id == AXP152_ID_DCDC3 ||ri->desc.id == AXP152_ID_DCDC4){
