...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(flag_state_change){
		...
		if(axp_debug){
    		DBG_PSY_MSG(DEBUG_SPLY,"Before Modify:Cap_Index = %d,val = 0x%x,pre_rest_vol = %d,rest_vol = %d\n",Cap_Index,val,pre_rest_vol,rest_vol);
		}
		...
		if(axp_debug){
    		DBG_PSY_MSG(DEBUG_SPLY,"After Modify:val = 0x%x,pre_rest_vol = %d,rest_vol = %d\n",val,pre_rest_vol,rest_vol);
		}
    	/* full */
...
//        	axp_set_bits(charger->master,0x32,0x38);
	...
	if(axp_debug && DEBUG_SPLY){
		for(i = 0; i < AXP20_VOL_MAX; i ++){
			DBG_PSY_MSG(DEBUG_SPLY,"Bat_Cap_Buffer[%d] = %d,Cap_Index = %d,cap_index_p = %d\n",i,Bat_Cap_Buffer[i],Cap_Index-1,cap_index_p);
		}
...
//#if defined (CONFIG_AXP_CHGCHANGE)
		...
		if(pmu_used2){
  		...
  		if(!gpio_adp_val){
  			ret = script_parser_fetch("pmu1_para", "pmu_init_chgcur2", &pmu_init_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_init_chgcur = INTCHGCUR / 1000;
      	}
      	pmu_init_chgcur = pmu_init_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_earlysuspend_chgcur2", &pmu_earlysuspend_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_earlysuspend_chgcur = SUSCHGCUR / 1000;
      	}
      	pmu_earlysuspend_chgcur = pmu_earlysuspend_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_suspend_chgcur2", &pmu_suspend_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_suspend_chgcur = SUSCHGCUR / 1000;
      	}
      	pmu_suspend_chgcur = pmu_suspend_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_resume_chgcur2", &pmu_resume_chgcur, sizeof(int));
      	if (ret){
        	printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_resume_chgcur = RESCHGCUR / 1000;
      	}
     		pmu_resume_chgcur = pmu_resume_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_shutdown_chgcur2", &pmu_shutdown_chgcur, sizeof(int));
      	if (ret){
        	printk("axp driver uning configuration failed(%d)\n", __LINE__);
      	...
      	pmu_shutdown_chgcur = pmu_shutdown_chgcur * 1000;
  			...
  			ret = script_parser_fetch("pmu1_para", "pmu_init_chgcur", &pmu_init_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_init_chgcur = INTCHGCUR / 1000;
      	}
      	pmu_init_chgcur = pmu_init_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_earlysuspend_chgcur", &pmu_earlysuspend_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_earlysuspend_chgcur = SUSCHGCUR / 1000;
      	}
      	pmu_earlysuspend_chgcur = pmu_earlysuspend_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_suspend_chgcur", &pmu_suspend_chgcur, sizeof(int));
      	if (ret){
      		printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_suspend_chgcur = SUSCHGCUR / 1000;
      	}
      	pmu_suspend_chgcur = pmu_suspend_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_resume_chgcur", &pmu_resume_chgcur, sizeof(int));
      	if (ret){
        	printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_resume_chgcur = RESCHGCUR / 1000;
      	}
      	pmu_resume_chgcur = pmu_resume_chgcur * 1000;
      	ret = script_parser_fetch("pmu1_para", "pmu_shutdown_chgcur", &pmu_shutdown_chgcur, sizeof(int));
      	if (ret){
        	printk("axp driver uning configuration failed(%d)\n", __LINE__);
        	pmu_shutdown_chgcur = CLSCHGCUR / 1000;
      	}
...
static int axp_battery_probe(struct platform_device *pdev)
{
  ...
  //var = script_get_item("pmu1_para", "pmu_used2", &item_val);
