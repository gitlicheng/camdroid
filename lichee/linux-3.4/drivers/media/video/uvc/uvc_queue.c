		...
		struct v4l2_buffer *v4l2_buf, int nonblocking)
{
	...
	static unsigned int max_byteused = 0;
	if(buf->buf.bytesused > max_byteused)
	{
		uvc_printk(KERN_DEBUG, "(f:%s, l:%d) max v4l2_buffer->buf.byteused[%d]!", __FUNCTION__, __LINE__, buf->buf.bytesused);
		max_byteused = buf->buf.bytesused;
	}
