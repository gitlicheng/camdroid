...
#define DRIVER_VERSION		"v1.0.8_SONiX_v2.6.36.04"
#else
#define DRIVER_VERSION		"v1.0.8_SONiX_v2.6.21.03"
#endif
		     ...
		     const struct usb_device_id *id)
{
	...
	// CHKuo copy from Houston 2011/06/20 Add & Map XU ctrl +++++
	// Adding & Mapping SONiX XU controls to uvc driver
	uvc_xu_ctrl_init(dev);

	// ------------------------------------------------
	// XU Ctrl: Asic read Chip ID
	dev->SONiX_Chip = 0;
	ret = uvc_xu_ctrll_ReadChip(dev, &dev->SONiX_Chip);

	if(dev->SONiX_Chip < 0)
		uvc_trace(UVC_TRACE_PROBE, "uvc_video_init(): Read chip failed \n");
	else
		uvc_trace(UVC_TRACE_PROBE, "uvc_video_init(): Read chip Success: %x\n", dev->SONiX_Chip);
	// Houston 2011/06/20 Add & Map XU ctrl -----
