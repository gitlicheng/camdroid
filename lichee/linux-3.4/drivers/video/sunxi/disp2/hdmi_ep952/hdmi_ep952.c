...
static int hdmi_parse_config(void)
{
    ...
    for(i=0; i < sizeof(hdmi_gpio) / sizeof(hdmi_gpio[0]); i++) {
        gpio_info = &(hdmi_gpio[i]);
        sprintf(io_name, "gpio_%d", i);
        ret = disp_sys_script_get_item(key_name, io_name, (int *)gpio_info, sizeof(disp_gpio_set_t)/sizeof(int));
        if(ret == 3) {
            hdmi_gpio_used[i]= 1;
            if(0 == gpio_info->mul_sel) { // input gpio
                hdmi_gpio_hdl[i] = disp_sys_gpio_request(gpio_info, 1);
            } else {
                hdmi_gpio_hdl[i] = 0;
            }
        }
...
static void ep952_update_hpd_status(void)
{
    ...
    unsigned char current_state =
        (1 == disp_sys_gpio_get_value(hdmi_gpio_hdl[1], "gpio_1")) ? 1 : 0;
#endif

    if(g_hpd_state != current_state) {
        if(s_hpd_change_count++ >= 20) {
            s_hpd_change_count = 0;
            g_hpd_state = current_state;
            //if(current_state) {
                //sunxi_edid_update(sunxi_edid); //fixme
            //}
...
void audio_config_para(void)
{
	EP_HDMI_Set_Audio_Fmt(AUD_I2S, AUD_SF_48000Hz);     // IIS input , 48KHz	
}
EXPORT_SYMBOL(audio_config_para);
...
static int __init ep952_module_init(void)
{
    ...
    if(1 == ret) {
       hdmi_used = value;
       if(hdmi_used) {
           ret = disp_sys_script_get_item(key_name, "hdmi_power",
               (int*)hdmi_power, sizeof(hdmi_power) / sizeof(int));
           if(2 == ret) {
               hdmi_power_used = 1;
               printk("[ep952] hdmi_power: %s\n", hdmi_power);
           }
           hdmi_i2c_init();
#if 0
