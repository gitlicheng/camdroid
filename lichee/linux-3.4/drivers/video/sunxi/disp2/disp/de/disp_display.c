...
s32 bsp_disp_sync_with_hw(disp_bsp_init_para * para)
{
	if((1 == para->boot_info.sync)
		...
		int disp_dev = disp;
		disp_output_type type = (disp_output_type)para->boot_info.type;
		disp_tv_mode mode = (disp_tv_mode)para->boot_info.mode;
		int ret = -1;
		struct disp_manager *mgr = NULL;
...
s32 bsp_disp_tv_register(disp_tv_func * func)
{
#if defined SUPPORT_TV
	u32 disp = 0;
	u32 num_screens = 0;
...
int bsp_disp_get_fb_info(unsigned int disp, disp_layer_info *info)
{
