...
static s32 tcon0_cfg_mode_tri(u32 sel, disp_panel_para * panel)
{
	...
	lcd_dev[sel]->tcon0_cpu_tri2.bits.trans_start_mode = 0;			// ecc fifo + tri fifo
	lcd_dev[sel]->tcon0_cpu_tri2.bits.sync_mode = 0;				// auto sync
	lcd_dev[sel]->tcon0_cpu_tri2.bits.start_delay = 3;				// 10
	/*
...
{
	disp_video_timings timing;

	memset(&timing, 0, sizeof(disp_video_timings));
