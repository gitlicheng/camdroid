...
#endif
{
	...
	if(disp_al_lcd_query_irq(disp, LCD_IRQ_TCON0_VBLK, &lcdp->panel_info)) {
		...
		if(cur_line <= (start_delay-4)) {
			sync_event_proc(disp, false);
		} else {
			sync_event_proc(disp, true);
		}
	}

	if(disp_al_lcd_query_irq(disp, LCD_IRQ_TCON0_CNTR, &lcdp->panel_info)
		|| disp_al_lcd_query_irq(disp, LCD_IRQ_TCON0_TRIF, &lcdp->panel_info)) {

		if (disp_al_lcd_tri_busy(disp, &lcdp->panel_info)) {
			/* if lcd is still busy when tri/cnt irq coming, take it as failture,
			 * record failture times, when it reach 2 times, clear counter
			 */
			lcdp->tri_finish_fail ++;
			lcdp->tri_finish_fail = (2 == lcdp->tri_finish_fail)?0:lcdp->tri_finish_fail;
		} else
			lcdp->tri_finish_fail = 0;

		/* when finish fail counter eq zero, sync event process,
		 * otherwise, notify timeout
		 */
		if (0 == lcdp->tri_finish_fail) {
			sync_event_proc(disp, false);
			disp_al_lcd_tri_start(disp, &lcdp->panel_info);
		} else
			sync_event_proc(disp, true);//timeout
	}

	return DISP_IRQ_RETURN;
}

static struct mutex mutex_lcd_enable;
static s32 disp_lcd_enable(struct disp_device* lcd)
{
	...
	mutex_lock(&mutex_lcd_enable);
	if(1 == disp_lcd_is_enabled(lcd)) {
		mutex_unlock(&mutex_lcd_enable);
		return 0;
	}
