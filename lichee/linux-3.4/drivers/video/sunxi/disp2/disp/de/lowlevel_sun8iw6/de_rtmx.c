...
int de_rtmx_set_route(unsigned int sel, unsigned char pno, unsigned int zoder)
{
	...
	__inf("addr=0x%x, reg=0x%x\n", (u32)&de200_rtmx[sel].bld_ctl->bld_route_ctl, de200_rtmx[sel].bld_ctl->bld_route_ctl.dwval);
...
int de_rtmx_set_background_color(unsigned int sel, unsigned int color)
{
	...
	__inf("bld_base=0x%x, bkcolor_base=0x%x, sel=%d, color=0x%x\n",
		(u32)de200_rtmx[sel].bld_ctl, (u32)&de200_rtmx[sel].bld_ctl->bld_bkcolor, sel, color);
...
int de_rtmx_sync_hw(unsigned int sel)
{
	...
	memcpy((void *)glb_ctl_block[sel].val, (void*)glb_ctl_block[sel].off,glb_ctl_block[sel].size);
	...
	for(j=0;j<vi_chno;j++)
	{
		...
		memcpy((void *)vi_fc_block[sel][j].val,(void *)vi_fc_block[sel][j].off,vi_fc_block[sel][j].size);

		memcpy((void *)vi_haddr_block[sel][j].val,(void *)vi_haddr_block[sel][j].off,vi_haddr_block[sel][j].size);

		memcpy((void *)vi_size_block[sel][j].val,(void *)vi_size_block[sel][j].off,vi_size_block[sel][j].size);
	}
	...
	for(j=0;j<ui_chno;j++)
	{
		for(i=0;i<layno;i++)
		{
			memcpy((void *)ui_attr_block[sel][j][i].val,(void *)ui_attr_block[sel][j][i].off,ui_attr_block[sel][j][i].size);
		}

		memcpy((void *)ui_haddr_block[sel][j].val,(void *)ui_haddr_block[sel][j].off,ui_haddr_block[sel][j].size);

		memcpy((void *)ui_size_block[sel][j].val,(void *)ui_size_block[sel][j].off,ui_size_block[sel][j].size);
	}

	memcpy((void *)bld_attr_block[sel].val,(void *)bld_attr_block[sel].off,bld_attr_block[sel].size);

	memcpy((void *)bld_ctl_block[sel].val,(void *)bld_ctl_block[sel].off,bld_ctl_block[sel].size);

	memcpy((void *)bld_ck_block[sel].val,(void *)bld_ck_block[sel].off,bld_ck_block[sel].size);

	memcpy((void *)bld_out_block[sel].val,(void *)bld_out_block[sel].off,bld_out_block[sel].size);
...
int de_rtmx_get_lay_size(unsigned int sel, unsigned int chno, unsigned int layno, disp_rectsz*size)
{
	int vi_chno = de_feat_get_num_vi_chns(sel);
	int ret = 0;
...
int de_rtmx_get_lay_win(unsigned int sel, unsigned int chno, unsigned int layno, disp_rect *win)
{
	int vi_chno = de_feat_get_num_vi_chns(sel);
	int ret = 0;
