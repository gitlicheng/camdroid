...
static disp_csc_type s_csc_type;
static disp_color	 s_color;
									   ...
									   scaler_para *ovl_cpara)
{
	...
	for (j=0;j<chn;j++)
	{
		...
		else if(yv12_d1_en[j][0]||yv12_d1_en[j][1]||yv12_d1_en[j][2]||yv12_d1_en[j][3]){
			pen[j] = de_rtmx_calc_chnrect(lay_en[j], layno, frame[j], crop[j], gsu_sel, p2p_para[j],
										  layer[j], &bld_rect[j], &ovlw[j], &ovlh[j]);
		}
		else{
...
int de_al_lyr_apply(unsigned int screen_id, struct disp_layer_config_data *data, unsigned int layer_num)
{
	...
	disp_rectsz dispsize[CHN_NUM] = {{0}};
...
//	layno = de_feat_get_num_layers(screen_id);
	...
	for (j=0;j<vi_chn;j++) {
		if(chn_used[j]) {
			...
			if (0 == isStraightYuv) {
				//__inf("ccsc self config DE_RGB.11111111111111111\n");
