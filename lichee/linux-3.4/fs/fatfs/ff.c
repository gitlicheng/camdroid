...
)
{
	...
	if( (fs->win[BS_55AA] != 0x55) || (fs->win[BS_55AA+1] != 0xaa) )
	{
		return 2;
	}

	if( (fs->win[BS_FilSysType] == 0x46) && (fs->win[BS_FilSysType+1] == 0x41)  &&  (fs->win[BS_FilSysType+2] == 0x54))
	{
		return 0;
	}
	
	if( (fs->win[BS_FilSysType32] == 0x46) && (fs->win[BS_FilSysType32+1] == 0x41)  &&  (fs->win[BS_FilSysType32+2] == 0x54))
	{
		return 0;
	}
...
)
{
    ...
    disk_ioctl(vol, GET_BLOCK_SIZE, &au);   /* set csize = 128 by force */
	if (!au) au = 1;
	if (au > 128) au = 128;
